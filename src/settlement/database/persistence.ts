...
import { fetchErc20Metadata } from '../smartContract';
...
const updatePortfolioForBatch = async (
  ...
  const matchRows = await client.query<{
    ...
    SELECT m.match_amount, m.lender_settlement_fee, m.borrower_settlement_fee,
           ...
           m.is_borrower_taker, m.maker_fee, m.taker_fee
  ...
  for (const row of matchRows.rows) {
    const lenderTradeFee = row.is_borrower_taker ? row.maker_fee : row.taker_fee;
    const borrowerTradeFee = row.is_borrower_taker ? row.taker_fee : row.maker_fee;

    // Build the two portfolio updates (lender deduct + borrower unlock)
    const lenderUpdate = {
      accountId: row.lender_account_id,
      query: `
        UPDATE portfolio
        SET amount = amount - ($1::numeric + $2::numeric + $3::numeric),
            locked_amount = GREATEST(0, locked_amount - ($1::numeric + $2::numeric + $3::numeric)),
            updated_at = NOW()
    ...
    const ordered = lenderUpdate.accountId < borrowerUnlock.accountId
      ? [lenderUpdate, borrowerUnlock]
      : [borrowerUnlock, lenderUpdate];

    for (const update of ordered) {
      await client.query(update.query, update.params);
    }

    // Add to borrower: amount += (matchedAmount - settlementFee - tradeFee), upsert
    await client.query(
      `
      INSERT INTO portfolio (id, account_id, asset_id, amount, locked_amount, is_collateral)
      VALUES (gen_random_uuid(), $1, $2, $3::numeric - $4::numeric - $5::numeric, 0, false)
      ON CONFLICT (account_id, asset_id) DO UPDATE SET
        amount = portfolio.amount + ($3::numeric - $4::numeric - $5::numeric),
        updated_at = NOW()
      `,
      [
        row.borrower_account_id,
  ...
  // Mark all matches in this batch as SETTLED
  await client.query(
    `
    UPDATE matches SET settlement_status = 'SETTLED', updated_at = NOW()
    WHERE id IN (
      SELECT si.match_id FROM settlement_items si WHERE si.settlement_batch_id = $1
    )
    `,
    [batchId],
  );
 ...
 */
export const processSettlementEvents = async (
  client: PoolClient,
  batchId: string,
